...
from lib.train.dataset import SV248S_TRAIN,SV248S_TRAIN_MODAL
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", 'SV248S_TRAIN','SV248S_TRAIN_MODAL']

        if name == "SV248S_TRAIN":
            if settings.use_lmdb:
                print("Building sv248s_train dataset from lmdb")
                datasets.append(SV248S_TRAIN(settings.env.sv248s_train_dir, image_loader=image_loader))
            else:
                datasets.append(SV248S_TRAIN(settings.env.sv248s_train_dir, image_loader=image_loader))
        if name == "SV248S_TRAIN_MODAL":
            datasets.append(SV248S_TRAIN_MODAL(settings.env.sv248s_train_modal_dir, dtype='rgbrgb'))
    return datasets
