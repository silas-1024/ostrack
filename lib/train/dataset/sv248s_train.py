...
class SV248S_TRAIN(BaseVideoDataset):
    """ TEST_SV dataset.
    """
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None):
        ...
        super().__init__('SV248S_TRAIN', root, image_loader)
    ...
    def _read_bb_anno(self, seq_path):
        bb_anno_file = os.path.join(seq_path, "groundTruth.rect")
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, '{:06}.tiff'.format(frame_id + 1))  # frames start from 1
    ...
    def get_class_name(self, seq_id):
        seq_path = self._get_sequence_path(seq_id).split("/")[-2]
