...
from ..layers.attn_blocks import CEBlock, candidate_elimination_adapter,CEBlock_version1
...
class Mona(nn.Module):
    ...
    def forward(self, x, hw_shapes=None):
        ...
        project1 = token2feature(project1)
        project1 = self.adapter_conv(project1)
        project1 = feature2token(project1)
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        for i in range(depth):
            ...
            # prompt_adapter.append(Adapter(D_features=embed_dim))
            # prompt_adapter.append(RepAdapter(in_features=embed_dim))
            # prompt_adapter.append(Mona(in_dim=embed_dim))
            # prompt_dropout.append(nn.Dropout(0.1))

        # self.prompt_adapter = nn.Sequential(*prompt_adapter)
        # self.prompt_dropout = nn.Sequential(*prompt_dropout)
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        z = self.patch_embed(z)
        x = self.patch_embed(x)
        z_dte = self.patch_embed_adapter(z_dte)
        x_dte = self.patch_embed_adapter(x_dte)
...
def vit_base_patch16_224_ce_adapter_modal(pretrained=False, **kwargs):
