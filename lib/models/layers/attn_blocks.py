...
def candidate_elimination_adapter(tokens: torch.Tensor, lens_t: int, global_index: torch.Tensor):
    ...
    attentive_tokens = tokens_s.gather(dim=1, index=global_index.unsqueeze(-1).expand(B, -1, C).long())
...
class CEBlock_version1(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # self.prompt_adapter = Adapter(D_features=dim)
        # self.mlp_adapter = Adapter(D_features=dim)
        self.prompt_adapter = RepAdapter(in_features=dim)
        self.mlp_adapter = RepAdapter(in_features=dim)
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        x_attn, attn = self.attn(self.norm1(self.prompt_adapter(x)), mask, True)
        ...
        x = x + self.drop_path(self.mlp(self.mlp_adapter(self.norm2(x))))
